  ...
  try {
    ...
    try {
      ...
      if (mlResult && typeof mlResult.estimated_price === 'number' && mlResult.estimated_price > 0) {
        ...
        logger.warn('[ML] Invalid price prediction response during search:', mlResult);
      }
    } catch (mlErr) {
      logger.warn('[ML] Price prediction unavailable during search, falling back to base pricing:', mlErr.message);
    }
